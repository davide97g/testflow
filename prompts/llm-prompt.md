...
test.describe('BAT-2076 - Event creation with existing appointments', () => {
  ...
  test.afterEach(async ({ singleResourcePracticeData }) => {
    // Cleanup
    await cancelAppointment(appointmentId, singleResourcePracticeData.practice.id);
  });
});
```
...
test.describe('BAT-T530 - Check In Appointment', () => {
  ...
  test.beforeEach(async ({ singleResourcePage, singleResourcePracticeData }) => {
    const { bookingApp } = useFeatures(singleResourcePage);
    await bookingApp.resetFilters();
    const { appointmentCreation } = useAppointmentFeatures(singleResourcePage, bookingApp);
    await appointmentCreation.createAppointment(singleResourcePracticeData);
    appointment = await AppointmentCreationFeature.getLatestAppointment(singleResourcePracticeData);
  });
  
  test.afterEach(async ({ singleResourcePracticeData }) => {
    await cancelAppointment(appointment?.id, singleResourcePracticeData.practice.id);
  });
});
```
...
test.afterEach(async ({ singleResourcePracticeData }) => {
  ...
  // Cleanup events
  if (eventId) {
    await deleteEventForASingleResource(eventId, singleResourcePracticeData.practice.id);
  }
...
test.beforeEach(async ({ singleResourcePracticeData }) => {
  const event = await createEventResource(singleResourcePracticeData.practice.id, {
